...
void CHARACTER_CORE::tick(bool use_input)
{
	...
	else if(hook_state == HOOK_FLYING)
	{
		...
		int hit = col_intersect_line(hook_pos, new_pos, &new_pos, 0, true);
		if(hit)
		{
	...
	if(hook_state == HOOK_GRABBED)
	{
		...
		hook_tick++;
		if(hooked_player != -1 && (hook_tick > SERVER_TICK_SPEED+SERVER_TICK_SPEED/5 || !world->characters[hooked_player]) && !super)
		{
			hooked_player = -1;
			hook_state = HOOK_RETRACTED;
			hook_pos = pos;			
