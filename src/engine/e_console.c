...
void console_execute_line_stroked(int stroke, const char *str,int lvl, int cid)
{
	PARSE_RESULT result;
	COMMAND *command;
		...
		if(command)
		{
			...
			if(stroke || is_stroke_command)
			{
				if(console_parse_args(&result, command->params))
				{
					...
					else if (command->lvl==100 && lvl<100)
					{
						char buf[256];
						str_format(buf, sizeof(buf), "You can't use this command: %s", command->name);
						server_send_rcon_line(cid, buf);
					}
					else
					{
						char buf[256];
						str_format(buf, sizeof(buf), "You have low level to use command: %s    Your level: %d    Need level: %d",command->name, lvl, command->lvl);
						server_send_rcon_line(cid, buf);
					}
			}
		}
...
void console_execute_line(const char *str,int lvl, int cid)
{
	console_execute_line_stroked(1, str,lvl, cid);
}
...
static void con_echo(void *result, void *user_data, int cid)
{
	console_print(console_arg_string(result, 0));
}

static void con_exec(void *result, void *user_data, int cid)
{
...
static void int_variable_command(void *result, void *user_data, int cid)
{
...
static void str_variable_command(void *result, void *user_data, int cid)
{
...
void console_init()
{
	MACRO_REGISTER_COMMAND("echo", "r", CFGFLAG_SERVER|CFGFLAG_CLIENT, con_echo, 0x0, "Echo the text",3);
	MACRO_REGISTER_COMMAND("exec", "r", CFGFLAG_SERVER|CFGFLAG_CLIENT, con_exec, 0x0, "Execute the specified file",3);

	#define MACRO_CONFIG_INT(name,def,min,max,flags,desc) { static INT_VARIABLE_DATA data = { &config_get_ ## name, &config_set_ ## name }; MACRO_REGISTER_COMMAND(#name, "?i", flags, int_variable_command, &data, desc,3) }
	#define MACRO_CONFIG_STR(name,len,def,flags,desc) { static STR_VARIABLE_DATA data = { &config_get_ ## name, &config_set_ ## name }; MACRO_REGISTER_COMMAND(#name, "?r", flags, str_variable_command, &data, desc,3) }
