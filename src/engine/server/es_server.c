...
static void con_unban(void *result, void *user_data, int cid)
{
	NETADDR addr;
	const char *str = console_arg_string(result, 0);
...
static void con_bans(void *result, void *user_data, int cid)
{
	int i;
	...
	for(i = 0; i < num; i++)
	{
		...
		{
			str_format(buf, sizeof(buf), "#%d %d.%d.%d.%d for life", i, addr.ip[0], addr.ip[1], addr.ip[2], addr.ip[3]);
			server_send_rcon_line(cid, buf);
		}
		else
		{
			t = info.expires - now;
			str_format(buf, sizeof(buf), "#%d %d.%d.%d.%d for %d minutes and %d seconds", i, addr.ip[0], addr.ip[1], addr.ip[2], addr.ip[3], t/60, t%60);
			server_send_rcon_line(cid, buf);
		}
	}
	str_format(buf, sizeof(buf), "%d ban(s)", num);
	server_send_rcon_line(cid, buf);
}

static void con_status(void *result, void *user_data, int cid)
{
	int i;
	NETADDR addr;
	char buf[512];
	for(i = 0; i < MAX_CLIENTS; i++)
	{
		if(clients[i].state == SRVCLIENT_STATE_INGAME)
		{
			...
			str_format(buf, sizeof(buf), "id=%d addr=%d.%d.%d.%d:%d name='%s' score=%d level=%d",
				i, addr.ip[0], addr.ip[1], addr.ip[2], addr.ip[3], addr.port,
				clients[i].name, clients[i].score, clients[i].authed);
			server_send_rcon_line(cid, buf);
		}
	}
}

static void con_shutdown(void *result, void *user_data, int cid)
{
	run_server = 0;
}

static void con_record(void *result, void *user_data, int cid)
{
	char filename[512];
...
static void con_stoprecord(void *result, void *user_data, int cid)
{
	demorec_record_stop();
}
...
static void server_register_commands()
{
	...
	MACRO_REGISTER_COMMAND("shutdown", "", CFGFLAG_SERVER, con_shutdown, 0, "",3);
