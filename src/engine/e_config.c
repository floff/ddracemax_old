...
void config_print()
{
	...
	#define MACRO_CONFIG_INT(name,def,min,max,flags,desc) if((flags)&CFGFLAG_SERVER){ str_format(linebuf, sizeof(linebuf), "%s %i", #name, config.name); }
	#define MACRO_CONFIG_STR(name,len,def,flags,desc) if((flags)&CFGFLAG_SERVER){ str_format(linebuf, sizeof(linebuf), "%s %s", #name, config.name); }
